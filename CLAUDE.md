...
Test files:
- `tests/test_selection.py` - 282 unit tests: ROM parsing, selection, filtering, config, playlists, transfers, size budget, english-only, multi-disc games, TOSEC naming, systems.json validation, IGDB integration, download throttle backoff, cross-platform dedup, standalone dedup analysis, version metadata, path resolution helpers
